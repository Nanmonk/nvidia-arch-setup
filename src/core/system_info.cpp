...
NvidiaArch SystemInfo::arch_from_name(const std::string& name) {
    ...
    // GTX 4xx / 5xx / Fermi
    if (n.find("gf1") != std::string::npos ||
        n.find("gtx 5") != std::string::npos || n.find("gtx 4") != std::string::npos)
...
std::string SystemInfo::driver_for_arch(NvidiaArch arch) {
    switch (arch) {
        ...
        case NvidiaArch::Kepler:    return "nvidia-470xx-dkms";
...
std::string SystemInfo::lib32_for_arch(NvidiaArch arch) {
    switch (arch) {
        ...
        case NvidiaArch::AdaLovelace:
        case NvidiaArch::Ampere:
        case NvidiaArch::Turing:    return "lib32-nvidia-utils";
        case NvidiaArch::Volta:
        case NvidiaArch::Pascal:
        case NvidiaArch::Maxwell:   return "lib32-nvidia-580xx-utils";
        case NvidiaArch::Kepler:    return "lib32-nvidia-470xx-utils";
        default:                    return "";
    }
}
...
std::optional<GpuInfo> SystemInfo::detect_nvidia() {
    ...
    for (auto& line : utils::split(res.stdout_str, '\n')) {
        ...
        if (l.find("nvidia") == std::string::npos) continue;
        ...
        if (!parts.empty()) gpu.pci_id = parts[0];
...
IgpuVendor SystemInfo::detect_igpu() {
    auto res = utils::exec("lspci -k -d ::03xx");
    if (res.exit_code != 0) return IgpuVendor::None;

    bool has_intel = false, has_amd = false;
    for (auto& line : utils::split(res.stdout_str, '\n')) {
        std::string l = to_lower(line);
        if (l.find("intel") != std::string::npos) has_intel = true;
        if (l.find("amd") != std::string::npos || l.find("advanced micro") != std::string::npos)
            has_amd = true;
    }
    if (has_intel) return IgpuVendor::Intel;
    if (has_amd)   return IgpuVendor::Amd;
...
KernelType SystemInfo::detect_kernel(std::string& ver_out) {
    ...
    if (v.find("-lts")      != std::string::npos) return KernelType::LinuxLts;
    if (v.find("-zen")      != std::string::npos) return KernelType::LinuxZen;
    if (v.find("-hardened") != std::string::npos) return KernelType::LinuxHardened;
    if (v.find("arch")      != std::string::npos) return KernelType::Linux;
    return KernelType::Custom;
}

Bootloader SystemInfo::detect_bootloader() {
    if (utils::file_exists("/etc/default/grub"))           return Bootloader::Grub;
    if (utils::file_exists("/boot/loader/loader.conf"))    return Bootloader::SystemdBoot;
    if (utils::file_exists("/boot/efi/EFI/refind"))        return Bootloader::Refind;
